...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [0, 255, 120]
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [0, 0, 255]
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [255, 0, 255]
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [70, 70, 220]
    mask_rgb[np.all(mask_convert == 6, axis=0)] = [102, 102, 156]
    mask_rgb[np.all(mask_convert == 7, axis=0)] = [190, 153, 153]
    mask_rgb[np.all(mask_convert == 8, axis=0)] = [180, 165, 180]
    return mask_rgb
