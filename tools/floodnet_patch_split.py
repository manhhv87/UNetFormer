...
#     mask_rgb[np.all(mask_convert == 0, axis=0)] = [255, 0, 0]       # label 0
#     mask_rgb[np.all(mask_convert == 1, axis=0)] = [0, 255, 0]       # label 1
#     mask_rgb[np.all(mask_convert == 2, axis=0)] = [0, 255, 120]     # label 2
#     mask_rgb[np.all(mask_convert == 3, axis=0)] = [0, 0, 255]       # label 3            
#     mask_rgb[np.all(mask_convert == 4, axis=0)] = [255, 0, 255]     # label 4
#     mask_rgb[np.all(mask_convert == 5, axis=0)] = [70, 70, 220]     # label 5
#     mask_rgb[np.all(mask_convert == 6, axis=0)] = [102, 102, 156]   # label 6
#     mask_rgb[np.all(mask_convert == 7, axis=0)] = [190, 153, 153]   # label 7
#     mask_rgb[np.all(mask_convert == 8, axis=0)] = [180, 165, 180]   # label 8
...
def rgb_to_2D_label(_label):
    ...
    label_seg[np.all(_label.transpose([1, 2, 0]) == BuildingFlooded, axis=-1)] = 0
    label_seg[np.all(_label.transpose([1, 2, 0]) == BuildingNonFlooded, axis=-1)] = 1
    label_seg[np.all(_label.transpose([1, 2, 0]) == RoadFlooded, axis=-1)] = 2
    label_seg[np.all(_label.transpose([1, 2, 0]) == RoadNonFlooded, axis=-1)] = 3
                ...
                if img_tile.shape[0] == split_size and img_tile.shape[1] == split_size and mask_tile.shape[0] == split_size and mask_tile.shape[1] == split_size:
                    ...
                    if cf[4] > 0.1 and mode == 'train':
                        ...
                        for i in range(len(car_imgs)):
                            out_img_path = os.path.join(imgs_output_dir, "{}_{}_{}_{}.jpg".format(img_filename, m, k, i))
                        ...
                        out_img_path = os.path.join(imgs_output_dir, "{}_{}_{}.jpg".format(img_filename, m, k))
