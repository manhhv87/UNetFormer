...
class FeatureRefinementHead(nn.Module):
    def __init__(self, in_channels=64, decode_channels=64):
        ...
        self.ca = nn.Sequential(nn.AdaptiveAvgPool2d(1),    # Global Average Pool
                                Conv(decode_channels, decode_channels // 16, kernel_size=1),    # Reduce
                                nn.ReLU6(),
                                Conv(decode_channels // 16, decode_channels, kernel_size=1),    # Expand
                                nn.Sigmoid())   
...
class AuxHead(nn.Module):
    ...
    def __init__(self, in_channels=64, num_classes=8):
        super().__init__()
        self.conv = ConvBNReLU(in_channels, in_channels)    # 3×3 convolution layer with batch normalization and ReLU
        self.drop = nn.Dropout(0.1)
        self.conv_out = Conv(in_channels, num_classes, kernel_size=1)   # 1×1 convolution layer

    def forward(self, x, h, w):
        feat = self.conv(x)
        feat = self.drop(feat)
        feat = self.conv_out(feat)
        feat = F.interpolate(feat, size=(h, w), mode='bilinear', align_corners=False)   # upsampling operation
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.b4 = GLTB(dim=decode_channels, num_heads=8, window_size=window_size)  # GLTB, number of heads h are both set to 8

        self.b3 = GLTB(dim=decode_channels, num_heads=8, window_size=window_size)

        self.p3 = WS(encoder_channels[-2], decode_channels)     # weight sum

        self.b2 = GLTB(dim=decode_channels, num_heads=8, window_size=window_size)
